...
//import Dealer from "../../../../BACKEND/models/dealer/dealer_acc_mgmt/dealer";
...
const ViewTopSellers = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${API_URL}/topdealer/`);
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`${API_URL}/topdealer/delete/${id}`);
